...
//    {
//        GLuint skyBoxID = mTextureManager->getTextureIDFromName(mCubeMapManager->GetSkyBoxName());
//
//        glActiveTexture(GL_TEXTURE0 + mCubeMapManager->GetSkyBoxTextureUnit());
//
//        glBindTexture(GL_TEXTURE_CUBE_MAP, skyBoxID);
//
//        GLint cubeMapSampler_UL = glGetUniformLocation(shaderProgramID, "cubeMapSampler");
//        glUniform1i(cubeMapSampler_UL, mCubeMapManager->GetSkyBoxTextureUnit());
//    }
//
...
void cControlGameEngine::AdjustAlphaTransparency(std::string modelName, float alphaTransparencyLevel)
{
    ...
    mSceneManager->CheckTransparency(meshTexture->sceneId);
}
...
//void cControlGameEngine::CreateFBO(int windowWidth, int windowHeight)
//{
...
//    }
//
//    FBOList.push_back(newFBO);
//}
//
//void cControlGameEngine::CreateDrawPass(bool isFBOTexture, glm::vec3 camEye, glm::vec3 camTarget, int FBOIndex)
//{
//    sDrawPass* newDrawPass = new sDrawPass(isFBOTexture, camEye, camTarget);
//
//    for (int index = 0; index < FBOList.size(); index++)
//    {
//        if (FBOIndex != 0 && index == FBOIndex - 1 )
//            newDrawPass->mFrameBufferObject = FBOList[index];
//    }
//
//    DrawPassList.push_back(newDrawPass);
//}
//
//void cControlGameEngine::SetMeshDrawPassIndex(std::string modelName, int drawPassIndex)
//{
//    cMesh* changeMeshDrawPassIndex = g_pFindMeshByFriendlyName(modelName);
//
//    changeMeshDrawPassIndex->drawPassIndex = drawPassIndex;
//}
//
//void cControlGameEngine::SetMeshTextureFromFBO(std::string modelName, bool isTextureFromFBO)
//{
//    cMesh* meshTextureFromFBO = g_pFindMeshByFriendlyName(modelName); 
//
//    meshTextureFromFBO->bTextureIsFromFBO = isTextureFromFBO;
//
//    if (isTextureFromFBO)
//        meshTextureFromFBO->mMixedTextures.textureRatiosList[0] = 1.0f;
//}
//
//void cControlGameEngine::SetFBOTextureNumber(std::string modelName, unsigned int FBOTextureNumber)
//{
//    cMesh* meshFBOTextureNumber = g_pFindMeshByFriendlyName(modelName);
//
//    for (int index = 0; index < FBOList.size(); index++)
//    {
//        if(FBOTextureNumber == index + 1 )
//            meshFBOTextureNumber->FBOTextureNumber = FBOList[index]->colourTexture_0_ID;
//    }
//}
...
int cControlGameEngine::InitializeGameEngine()
{
    ...
    mSceneManager->InitializeSceneManager(mVAOManager, mTextureManager, mCubeMapManager);
...
void cControlGameEngine::RunGameEngine(GLFWwindow* window)
{
    ...
    this->mSceneManager->LoadScene(shaderProgramID, window, this->cameraEye, this->cameraTarget);
